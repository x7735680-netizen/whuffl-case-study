...
export default function BuildAppCarousel() {
  ...
  return <div className="build-app-experience"><div className="app-carousel" ref={carousel} aria-label="Whuffl app screens">{SCREENS.map((screen, index) => <figure className={`app-slide ${Math.abs(index - centerIndex) <= 1 ? "is-near" : ""}`} data-index={index} ref={(node) => { slides.current[index] = node; }} key={screen.file}><img data-preload-src={APP(screen.file)} alt={screen.title} /><figcaption>{screen.title}</figcaption></figure>)}</div><ol className="experience-flow" aria-label="Five-step experience flow">{STEPS.map((step) => <li key={step.id} className={activeStep === step.id ? "is-active" : ""}><button type="button" onClick={() => reveal(step.screen)} aria-current={activeStep === step.id ? "step" : undefined}><span className="experience-flow-dot" /><strong>{step.title}</strong><small>{step.body}</small></button></li>)}</ol></div>;
}
